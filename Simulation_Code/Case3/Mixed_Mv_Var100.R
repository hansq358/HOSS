# HOSS Simulation Case 3: Mixed MVN
...
mysimu = function(R, sigma_a2, sigma_e2, B, n_values) {
  ...
  Sigma_G <- diag(0.5, N_G_CONT, N_G_CONT) + matrix(0.5, N_G_CONT, N_G_CONT)
  Sigma_I <- diag(0.5, N_I_CONT, N_I_CONT) + matrix(0.5, N_I_CONT, N_I_CONT)
  ...
  for (n in n_values) {
    ...
    for (k in 1:B) {
      ...
      Z_group_cont_raw <- mvrnorm(R, rep(0, N_G_CONT), Sigma_G)
      ...
      for (i in 1:R) { X_ind_cont[((C_cum[i] + 1):C_cum[i + 1]), ] <- mvrnorm(C_i[i], rep(-1 + i / (R/2), N_I_CONT), Sigma_I) }
...
write.csv(simulation_results[[1]], file = "simulation_results_Case3_Var100.csv")
