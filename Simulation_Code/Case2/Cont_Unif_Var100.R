# HOSS Simulation Case 2: Continuous Uniform
...
mysimu = function(R, sigma_a2, sigma_e2, B, n_values) {
  ...
  for (n in n_values) {
    ...
    for (k in 1:B) {
      ...
      Z_group_raw <- matrix(runif(R*N_G, -1, 1), R, N_G)
      ...
      for (i in 1:R) { 
        # Shift mean to create heterogeneity
        X_ind[((C_cum[i] + 1):C_cum[i + 1]), ] <- matrix(runif(C_i[i]*N_I, -2+i/(R/2), 0+i/(R/2)), C_i[i], N_I) 
      }
      ...
      group_pure_complex <- grep("z_g", complex_col_names, value=TRUE)
      group_pure_complex <- grep("x_i", group_pure_complex, value=TRUE, invert=TRUE)
      
      # Filter for pure individual-level complex terms
      ind_pure_complex <- grep("x_i", complex_col_names, value=TRUE)
      ind_pure_complex <- grep("z_g", ind_pure_complex, value=TRUE, invert=TRUE)
      
      # Select specific terms to be Strong (0.5)
      active_group_names <- group_pure_complex[1:min(length(group_pure_complex), N_G_active_complex)]
      active_ind_names <- ind_pure_complex[1:min(length(ind_pure_complex), N_I_active_complex)]
      
      idx_group_active <- match(active_group_names, colnames(X_full))
      idx_ind_active <- match(active_ind_names, colnames(X_full))
...
write.csv(simulation_results[[1]], file = "simulation_results_Case2_Var100.csv")
